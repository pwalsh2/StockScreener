...
export const FilterOtherNonCurrentLiabilities = (props) => {
	// props passed in:
	...
	const filterData = (e) => {
		...
		if (twoFields) {
			...
			newestData = GlobalDataCopy.filter(function (d) {
				return d.OtherNonCurrentLiabilities > tempVal1 && d.OtherNonCurrentLiabilities < tempVal2;
			});
		} else {
			var tempVal1 = parseInt(val1); // input states are string must parse
			if (operator === ">") {
				// filter data
				newestData = GlobalDataCopy.filter(function (d) {
					return d.OtherNonCurrentLiabilities > tempVal1;
				});
			} else {
				newestData = GlobalDataCopy.filter(function (d) {
					return d.OtherNonCurrentLiabilities < tempVal1;
