...
export const FilterDeferedTaxLiabilitesNonCurrent = (props) => {
	// props passed in:
	...
	const filterData = (e) => {
		...
		if (twoFields) {
			...
			newestData = GlobalDataCopy.filter(function (d) {
				return d.DeferedTaxLiabilitesNonCurrent > tempVal1 && d.DeferedTaxLiabilitesNonCurrent < tempVal2;
			});
		} else {
			var tempVal1 = parseInt(val1); // input states are string must parse
			if (operator === ">") {
				// filter data
				newestData = GlobalDataCopy.filter(function (d) {
					return d.DeferedTaxLiabilitesNonCurrent > tempVal1;
				});
			} else {
				newestData = GlobalDataCopy.filter(function (d) {
					return d.DeferedTaxLiabilitesNonCurrent < tempVal1;
