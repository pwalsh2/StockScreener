...
export const FilterThreeYrReceiveblesGrowth = (props) => {
	// props passed in:
	...
	const filterData = (e) => {
		...
		if (twoFields) {
			...
			newestData = GlobalDataCopy.filter(function (d) {
				return d.ThreeYrReceiveblesGrowth > tempVal1 && d.ThreeYrReceiveblesGrowth < tempVal2;
			});
		} else {
			var tempVal1 = parseInt(val1); // input states are string must parse
			if (operator === ">") {
				// filter data
				newestData = GlobalDataCopy.filter(function (d) {
					return d.ThreeYrReceiveblesGrowth > tempVal1;
				});
			} else {
				newestData = GlobalDataCopy.filter(function (d) {
					return d.ThreeYrReceiveblesGrowth < tempVal1;
